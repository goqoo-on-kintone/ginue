...
- **レポートのOAuth対応**: `reports.json`がOAuth 2.0認証に対応しました。`skipOauth`フラグを削除し、OAuth認証時にもレポート設定がスキップされなくなりました。

### 内部

- **依存関係**: `gyuma`を0.6.3にアップグレード。
...
- **Windowsインストール**: `gyuma`を0.6.2にアップグレード。gyumaの`preinstall`で`typesync || true`が実行されなくなり、Windowsでも問題なくインストールできるようになりました。
