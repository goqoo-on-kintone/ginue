...
exports.ginuePull = async (ktn, opts) => {
  ...
  if (ktn.command === 'app/customize.json' && opts.downloadJs) {
    ...
    const desktopFileInfos = [customizeInfo.desktop.js, customizeInfo.desktop.css].flatMap((infos) =>
      infos.filter((info) => info.type === 'FILE').map((info) => ({ ...info.file, target: 'desktop' }))
    )
    const mobileFileInfos = [customizeInfo.mobile.js, customizeInfo.mobile.css].flatMap((infos) =>
      infos.filter((info) => info.type === 'FILE').map((info) => ({ ...info.file, target: 'mobile' }))
    )
    const fileInfos = [...desktopFileInfos, ...mobileFileInfos]
    ...
    for (const jsFileInfo of fileInfos) {
      ...
      const client = new KintoneRestAPIClient({
        ...
        basicAuth: {
          username: opts.basic_user,
          password: opts.basic_password,
        },
      })

      const jsFile = await client.file.downloadFile({ fileKey })
      const jsFilePath = `${dirPath}/${target}/${name}`
      mkdirp.sync(path.dirname(jsFilePath))
      fs.writeFileSync(jsFilePath, jsFile.toString())
      console.log(jsFilePath)
      console.log('CUSTOMIZE!', { fileKey, name })
    }
  }
}
