...
const sortObj = (ktn, kintoneInfoObj) => {
  switch (ktn.command) {
    ...
    case 'app/views.json': {
      const views = Object.entries(_.cloneDeep(kintoneInfoObj.views))
      ...
      return indexes.reduce((obj, index) => {
        const [key, value] = views.find(([, value]) => value.index === index)
        obj[key] = value
        return obj
      }, {})
    }
...
const convertKintoneInfo = (kintoneInfoObj, ktn, opts) => {
  ...
  const kintoneInfo = prettyln(sortObj(ktn, kintoneInfoObj))

  let kintoneInfoAlt
  if (opts.alt) {
    const isConverted = convertAppIdToName(ktn, kintoneInfoObj)
    kintoneInfoAlt = isConverted ? prettyln(sortObj(ktn, kintoneInfoObj)) : undefined
  }
