...
exports.ginuePull = async (ktn, opts) => {
  ...
  if (ktn.command === 'app/customize.json' && opts.downloadJs) {
    const jsFileInfos = JSON.parse(kintoneInfo).desktop.js.filter((_) => _.type === 'FILE')
    for (const jsFileInfo of jsFileInfos) {
      const { fileKey, name } = jsFileInfo.file
      ...
      const jsFilePath = `kintone-settings/production/js/${name}`
