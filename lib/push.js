...
const confirmDeleteFieldInRoot = async (ktn, kintoneInfo, message) => {
...
const confirmDeleteFieldInSubtable = async (ktn, kintoneInfo, message) => {
  ...
  try {
    ...
    if (ktn.command === 'preview/app/form/fields.json' && e.error.code === 'GAIA_FC01') {
      ...
      if (e.error.code === 'GAIA_FN11') {
        ...
        fieldsToDelete = await confirmDeleteFieldInRoot(ktn, kintoneInfo, message)
      } else if (e.error.code === 'CB_VA01') {
        // サブテーブルフィールドの削除(指定するフィールドに不足がある、またはテーブルにないフィールドを指定しています)
        fieldsToDelete = await confirmDeleteFieldInSubtable(ktn, kintoneInfo, JSON.stringify(errors))
      }

      if (fieldsToDelete) {
        await deleteFields(ktn, kintoneInfo, fieldsToDelete).catch(() => {
          throw e
        })
        await execPush(ktn, kintoneInfo)
      } else {
