...
const confirmDeleteFieldsInRoot = async (ktn, kintoneInfo, message) => {
...
const confirmDeleteFieldsInSubtable = async (ktn, kintoneInfo, message) => {
  ...
  try {
    ...
    if (ktn.command === 'preview/app/form/fields.json' && e.error.code === 'GAIA_FC01') {
      ...
      const confirmDeleteFields = {
        GAIA_FN11: () => confirmDeleteFieldsInRoot(ktn, kintoneInfo, message),
        CB_VA01: () => confirmDeleteFieldsInSubtable(ktn, kintoneInfo, JSON.stringify(errors)),
      }[e.error.code]
