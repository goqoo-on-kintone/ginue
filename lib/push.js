...
const deleteField = async (ktn, kintoneInfo, fields) => {
  const deletingKtn = {
    ...ktn,
...
const deleteFieldInRoot = async (ktn, kintoneInfo, message) => {
  ...
  await deleteField(ktn, kintoneInfo, fields)
}
  ...
  try {
    ...
    if (ktn.command === 'preview/app/form/fields.json' && e.error.code === 'GAIA_FC01') {
      ...
      if (e.error.code === 'GAIA_FN11') {
        ...
        await deleteFieldInRoot(ktn, kintoneInfo, message).catch(() => {
          throw e
        })
        await execPush(ktn, kintoneInfo)
