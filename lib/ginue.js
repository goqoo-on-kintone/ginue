...
const createPushTargetAppMap = opts => {
  ...
  for (const [appName, pushTargetAppId] of Object.entries(opts.pushTarget.app)) {
    const pushBaseAppId = opts.apps[appName]
    if (!pushBaseAppId) {
      console.error(`ERROR: App "${appName}" not found in "${opts.pushTarget.environment}" environment!`)
      process.exit(1)
    }
    pushTargetAppMap[pushBaseAppId] = pushTargetAppId
  }
