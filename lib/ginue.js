...
const loadGinuerc = async () => {
  ...
  return Object.entries(ginuerc.env).map(([envName, prop]) => {
    prop.environment = envName
    if (ginuerc.location) {
      prop.locationRoot = ginuerc.location
    }
    return prop
  })
}
...
const parseArgumentOptions = () => {
  const argv = minimist(process.argv.slice(2), {
    boolean: ['version', 'help', 'preview', 'acl', 'js', 'alt'],
    string: ['location', 'domain', 'username', 'password', 'app', 'guest', 'basic', 'exclude'],
    alias: {
      v: 'version',
      h: 'help',
      l: 'location',
