...
const loadGinuerc = async () => {
  ...
  return Object.entries(ginuerc.env).map(([envName, innerGinuerc]) => {
    innerGinuerc.environment = envName

    if (ginuerc.location) {
      innerGinuerc.locationRoot = ginuerc.location
    }

    ;['fileType', 'preview', 'alt'].forEach(prop => {
      if (ginuerc[prop] && innerGinuerc[prop] === undefined) {
        innerGinuerc[prop] = ginuerc[prop]
      }
    })
...
const createDirPath = (ktn, opts) => {
  ...
  if (opts.locationRoot) {
    dirPath += `${opts.locationRoot}/`
  }
...
const convertAppFormFieldsJson = (properties, apps) => {
  for (const prop of Object.values(properties)) {
    if (prop.lookup) {
      const relatedApp = prop.lookup.relatedApp
      relatedApp.app = apps ? apps[relatedApp.app] : '<APP_ID>'
    }
    ...
    if (prop.fields) {
      convertAppFormFieldsJson(prop.fields, apps)
    }
  }
}
...
const parseArgumentOptions = () => {
  const argv = minimist(process.argv.slice(2), {
    ...
    string: ['location', 'domain', 'username', 'password', 'app', 'guest', 'basic', 'exclude', 'fileType'],
    alias: {
      v: 'version',
      h: 'help',
      l: 'location',
  ...
  if (argv.location || argv.domain || argv.username || argv.password || argv.app || argv.guest) {
    argv.priority = true
  }
...
const createOptionValues = async () => {
  ...
  if (!argv.type || !['pull', 'push', 'reset', 'deploy'].includes(argv.type)) {
    usageExit(1)
  }
...
const createPushTargetAppMap = opts => {
  const pushTargetAppMap = {}
  for (const [appName, appId] of Object.entries(opts.apps)) {
    const pushTargetApp = opts.pushTarget.app[appName]
    if (!pushTargetApp) {
      console.error(`ERROR: App "${appName}" not found in "${opts.pushTarget.environment}" environment!`)
      process.exit(1)
    }
    pushTargetAppMap[appId] = pushTargetApp
  }
  return pushTargetAppMap
}
...
const ginuePush = async (ktn, opts, pushTarget) => {
  ...
  if (pushTarget) {
    if (ktn.command === 'preview/app/form/fields.json') {
      convertAppFormFieldsJson(kintoneInfo.properties, createPushTargetAppMap(opts))
    }
