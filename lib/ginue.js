...
const loadKintoneCommands = async (exclude) => {
  ...
  const commandsJsonPath = path.join(__dirname, 'commands')
  const kintoneCommands = await loadRequiedFile(commandsJsonPath)
  if (exclude) {
...
const ginuePush = async (ktn, opts, pushTarget) => {
  ...
  const kintoneInfo = await loadRequiedFile(filePath)
  ktn.command = `preview/${ktn.command}`
